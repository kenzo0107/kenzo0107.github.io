---
layout: post
title: Checking Your Google Analytics Tag Setup
date: 2015-03-24
lang: en
translation_id: check-google-analytics-tag
permalink: en/2015/03/24/check-google-analytics-tag/
cover: https://cdn-ak.f.st-hatena.com/images/fotolife/k/kenzo0107/20150324/20150324104431.png
---

## Steps

### Install Tag Assistant from the Google Chrome extension store.
It shows up in the toolbar area as follows.

![](http://cdn-ak.f.st-hatena.com/images/fotolife/k/kenzo0107/20150324/20150324104431.png)


### Visit the page where the tag is configured and click the Tag Assistant icon


- When a Remarketing tag is configured

![](http://cdn-ak.f.st-hatena.com/images/fotolife/k/kenzo0107/20150324/20150324104445.png)




- When Remarketing and purchase tags are configured

![](http://cdn-ak.f.st-hatena.com/images/fotolife/k/kenzo0107/20150324/20150324104454.png)



That's all.
