...
This is a sandbox for the DB migration tool `Phinx`.
...
The container `db-migrate` is used as a one-off container, so its state is `Exit 0`.
...
By default, Phinx selects the `development` environment.
