...
- Create an MFA-enabled profile with awsume
  - By using `--output-profile claude`, you can consolidate multiple profiles into a single temporary profile named `claude` ⭐️

```console
awsume dummy --output-profile claude
```

## Checking the connected AWS account ID with Claude Code + AWS MCP
...
<details>
<summary>Example: Tell me the AWS account ID</summary>

```console
⏺ AWS CLI機能を使用してAWSアカウントIDを取得します。

⏺ aws-mcp - aws___call_aws (MCP)(cli_command: "aws sts get-caller-identity")
  ⎿  {
       "response": {
         "status_code": 200,
...
⏺ AWSアカウント情報を取得しました：
  ...
  詳細情報:
  - ユーザーID: ABCDEFGHIJKLMNOPQRSTU:kenzo.tanaka
  - ARN: arn:aws:sts::123456789012:assumed-role/admin/kenzo.tanaka
  - ロール: admin

  現在は管理者権限のロールでAWSアカウント 123456789012 にアクセスしています。
```

</details>

## Investigating logs with Claude Code + CloudWatch Logs MCP

This was deprecated on October 13, 2025.
https://pypi.org/project/awslabs.cloudwatch-logs-mcp-server/

Switching to [awslabs.cloudwatch-mcp-server](https://github.com/awslabs/mcp/tree/main/src/cloudwatch-mcp-server) is recommended.
...
<details>
<summary>Example: List the log groups</summary>
...
## Investigating costs with Claude Code + Cost Explorer MCP
...
<details>
<summary>Example: Using the aws-ce MCP, what was last month's bill for AWS account 123456789012?</summary>
...
## Checking pricing with Claude Code + AWS Pricing MCP
...
<details>
<summary>Example: Using the aws-price MCP, calculate which is cheaper between Aurora Serverless v2 and two db.t4g.medium instances on a 1-year All Upfront Reserved Instance, and find the break-even point.</summary>
...
## Referencing documentation with Claude Code + AWS Documentation MCP
...
<summary>Example: Using the aws-docs MCP, tell me the CloudFront configuration for serving S3 objects via CloudFront signed URLs</summary>
...
## Implementing with Claude Code + AWS Terraform MCP
...
## Having it draw a simple architecture diagram with Claude Code + AWS Diagram MCP
...
<details>
<summary>Using the aws-diagram MCP, draw a simple architecture diagram of NAT Gateway for both regional and zonal configurations</summary>

## Basic NAT Gateway Architecture
...
## Roughly checking for recent incidents with Claude Code + AWS CloudWatch + AWS Application Signals MCP
...
<details>
<summary>Using the aws-appsignals MCP, please check whether there are any recent incidents.</summary>

```console
  AWS Application Signalsの調査により、以下の状況が確認されました：

  🚨 検出された問題

  dummy-backend サービスで高レイテンシを検出

  1. GET /health エンドポイント
    - 現在のレイテンシ: 55.82ms（p99）
    - 正常ベースライン: 1.58ms
    - 重要度: HIGH
    - 問題: 通常の35倍以上のレイテンシ
  2. InternalOperation
    - 現在のレイテンシ: 843.38ms（p99）
    - 正常ベースライン: 281.20ms
    - 重要度: HIGH
    - 問題: 通常の3倍のレイテンシ
  ...
  1. 即座の対応
    - dummy-backendサービスのパフォーマンス調査
    - ヘルスチェックエンドポイントの応答遅延原因特定
  2. 中期的対応
    - SLOの設定を検討（現在未設定）
    - アプリケーションログの詳細分析

  現在進行中のインシデントとして、ステージング環境のバックエンドサービスでレイテンシ問題が発生していることが確認されました。
```
</details>
